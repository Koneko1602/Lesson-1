...
export const videosInputDtoValidation = (
    ...
    if (!Array.isArray(data.availableResolutions)) {
        ...
        const invalidValues = data.availableResolutions.filter(
            (res) => typeof res !== 'string' || !allowedResolutions.includes(res)
        );
    ...
    if (
        data.publicationDate !== undefined // если поле есть
    ) {
        if (
